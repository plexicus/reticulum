...
class DeploymentTargetGenerator:
    ...
    def generate_deployment_targets(self, changed_files: List[Path]) -> List[DeploymentTarget]:
        """
        ...
        affected_services = self.reverse_ownership_index.find_services_by_file_change(changed_files)
    ...
    def _group_files_by_service(self, affected_services: Dict[Path, List[str]]) -> Dict[str, List[Path]]:
        """
    ...
    def _resolve_services_to_charts(self, service_to_files: Dict[str, List[Path]]) -> Dict[str, dict]:
        """
        ...
        for service_token, files in service_to_files.items():
            ...
            if chart_name:
                ...
                mapping = self.chart_resolver.service_chart_registry.get_mapping_details(service_token)
                if mapping:
                    chart_targets[chart_name]['confidence'] = max(
                        chart_targets[chart_name]['confidence'],
                        mapping.confidence
                    )

        return chart_targets

    def _create_deployment_targets(self, chart_targets: Dict[str, dict]) -> List[DeploymentTarget]:
        """
    ...
    def get_deployment_summary(self, deployment_targets: List[DeploymentTarget]) -> Dict[str, any]:
        """
        ...
        total_services = sum(len(target.affected_services) for target in deployment_targets)
    ...
    def analyze_impact(self, changed_files: List[Path]) -> Dict[str, any]:
        ...
        return {
            ...
            'file_impact_ratio': files_with_owners / total_files if total_files > 0 else 0,
            'service_impact_ratio': len(all_services) / len(self.chart_resolver.service_chart_registry.service_to_chart) if self.chart_resolver.service_chart_registry.service_to_chart else 0
        }