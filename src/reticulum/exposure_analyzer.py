...
class ExposureAnalyzer:
    ...
    def analyze_chart(self, chart_dir: Path, repo_path: Path) -> Dict[str, Any]:
        ...
        if chart_info["containers"]:
            chart_info["containers"] = self._consolidate_exposures(chart_info["containers"])
    ...
    def _analyze_parsed_content(
        ...
        self._search_recursive(content, chart_name, chart_dir, repo_path, file_type, containers)
    ...
    def _search_recursive(
        ...
        if isinstance(obj, dict):
            for key, value in obj.items():
                ...
                # Continue recursion
                self._search_recursive(value, chart_name, chart_dir, repo_path, file_type, containers, current_path)
                
        elif isinstance(obj, list):
            for i, item in enumerate(obj):
                current_path = f"{path}[{i}]"
                self._search_recursive(item, chart_name, chart_dir, repo_path, file_type, containers, current_path)
    ...
    def _consolidate_exposures(self, containers: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for container in containers:
            ...
            if chart_name not in consolidated:
                ...
                existing["exposure_score"] = max(existing["exposure_score"], container["exposure_score"])
        ...
        for chart_name, consolidated_data in consolidated.items():
            ...
            if len(consolidated_data["gateway_types"]) > 1:
                container["gateway_type"] = f"Multiple: {', '.join(consolidated_data['gateway_types'])}"
            else:
            ...
            if len(consolidated_data["hosts"]) > 1:
                container["host"] = f"Multiple configurations: {' | '.join(consolidated_data['hosts'])}"
            else:
