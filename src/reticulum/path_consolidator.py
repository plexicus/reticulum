...
class PathConsolidator:
    ...
    def build_master_paths(self, containers: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        consolidated_paths = self._consolidate_master_paths(list(path_containers.keys()))
        ...
        for master_path in consolidated_paths:
            ...
            if related_containers:
                ...
                master_paths[master_path] = {
                    ...
                    "primary_container": most_exposed["name"],  # Primary container for reference
    ...
    def _consolidate_master_paths(self, paths: List[str]) -> List[str]:
        ...
        for path in sorted_paths:
            ...
            for existing_path in consolidated:
                ...
                if norm_path.startswith(norm_existing + "/") or norm_path == norm_existing:
                    is_subpath = True
                    break
            
            if not is_subpath:
                # Remove any existing paths that are children of this path
                consolidated = [p for p in consolidated if not p.rstrip('/').startswith(norm_path + "/")]
    ...
    def _find_most_exposed_container(self, containers: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        sorted_containers = sorted(
            containers, 
            key=lambda c: (-c.get("exposure_score", 0), c.get("name", ""))
        )
