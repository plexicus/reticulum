...
class AlternativeEvidenceAnalyzer:
    ...
    def discover_services_without_dockerfiles(
        ...
        for chart_dir in chart_dirs:
            ...
            if evidence["has_concrete_evidence"]:
                service = self._create_service_from_evidence(
                    chart_dir, chart_name, evidence
                )
                discovered_services.append(service)

        return discovered_services

    def _collect_evidence_for_chart(
        self, chart_dir: Path, chart_name: str
    ) -> Dict[str, Any]:
                ...
                try:
                    ...
                    if compose_data and "services" in compose_data:
                        for service_name, service_config in compose_data[
                            ...
                            if service_name == chart_name or service_name.replace(
                                "-", "_"
                                ...
                                evidence.append(
                                    {
                                        "type": "docker_compose",
                                        "file": str(
                                            compose_path.relative_to(self.repo_root)
                                        ),
                                        "service_name": service_name,
                                        "build_context": service_config.get(
                                            "build", {}
                                        ).get("context", ""),
    ...
    def _analyze_kubernetes_manifests(
        self, chart_dir: Path, chart_name: str
    ) -> List[Dict[str, Any]]:
                ...
                try:
                    ...
                    if manifest_data:
                        ...
                        if kind in ["Deployment", "StatefulSet", "DaemonSet"]:
                            ...
                            if name == chart_name or name.replace(
                                "-", "_"
                            ) == chart_name.replace("-", "_"):
                                evidence.append(
                                    {
                                        ...
                                        "file": str(
                                            manifest_path.relative_to(self.repo_root)
                                        ),
                                        "kind": kind,
                                        "name": name,
                                        "confidence": "high",
    ...
    def _analyze_build_scripts(
        ...
        build_scripts = [
            ...
            chart_dir / "build.sh",
            chart_dir
            / "Dockerfile",  # Check if Dockerfile exists but wasn't found by DockerfileAnalyzer
        ]
            ...
            for cicd_path in self.repo_root.glob(pattern):
                if cicd_path.exists():
                    ...
                    try:
                        ...
                        if (
                            chart_name in content
                            or chart_name.replace("-", "_") in content
                        ):
                            evidence.append(
                                {
                                    "type": "cicd_pipeline",
    ...
    def _analyze_package_files(
                            ...
                            evidence.append(
                                {
                                    "type": "package_file",
                                    "file": str(
                                        package_file.relative_to(self.repo_root)
                                    ),
                                    "source_directory": str(
                                        source_dir.relative_to(self.repo_root)
                                    ),
                                    "confidence": "high",
                                }
                            )
                            source_paths.append(
                                str(source_dir.relative_to(self.repo_root)) + "/"
                            )
                                ...
                                evidence.append(
                                    {
                                        ...
                                        "file": str(
                                            config_file.relative_to(self.repo_root)
                                        ),
                                        "source_directory": str(
                                            source_dir.relative_to(self.repo_root)
                                        ),
        ...
        return {"evidence": evidence, "source_paths": source_paths}
    ...
    def _analyze_configuration_files(
        self, chart_dir: Path, chart_name: str
    ) -> Dict[str, Any]:
            ...
            for config_path in self.repo_root.glob(pattern):
                if config_path.exists():
                    try:
                        ...
                        if (
                            chart_name in config_str
                            or chart_name.replace("-", "_") in config_str
                        ):
                            evidence.append(
                                {
                            ...
                            source_dirs = self._extract_source_paths_from_config(
                                config_data
                            )
                ...
                try:
                    ...
                    if values_data:
                        ...
                        if "service" in values_data:
                            service_info["service_type"] = values_data["service"].get(
                                "type", ""
                            )
                            service_info["service_port"] = values_data["service"].get(
                                "port", ""
                            )
        ...
        if isinstance(config_data, dict):
            ...
            for key, value in config_data.items():
                if isinstance(value, str) and any(
                    ...
                    for path_indicator in ["path", "dir", "source", "src"]
                ):
