...
class DockerfileAnalyzer:
    ...
    def find_dockerfile(self, chart_dir: Path, repo_path: Path, chart_name: str) -> Optional[Path]:
    ...
    def parse_dockerfile_for_source_paths(self, dockerfile_path: Path, repo_root: Path) -> List[str]:
        ...
        try:
            ...
            for line in lines:
                ...
                copy_match = re.match(r'(?:COPY|ADD)\s+([^\s]+)\s+([^\s]+)', line, re.IGNORECASE)
                if copy_match:
                    ...
                    if clean_source == '.' and clean_dest.startswith('/app'):
                        ...
                        raw_paths.append('.')
                    elif (clean_source and 
                          clean_source != '.' and  # Don't capture '.' when it's not a full dir copy
                          not clean_source.startswith(('--', 'http://', 'https://', '$')) and
                          not clean_source.endswith('.tar')):
    ...
    def _consolidate_source_paths(self, raw_paths: List[str], repo_root: Path) -> List[str]:
        ...
        for path in raw_paths:
            if path:
                ...
                if '/' in clean_path:
                    ...
                    if dir_part and dir_part not in ['app', 'usr', 'opt']:  # Skip system dirs
