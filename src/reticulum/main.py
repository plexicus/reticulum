...
class ExposureScanner:
    ...
    def scan_repo(self, repo_path: str) -> Dict[str, Any]:
        ...
        if not charts:
            self.results["scan_summary"] = {"error": "No Helm charts found in repository"}
            return self.results
        ...
        for chart_file in charts:
            ...
            self.chart_containers[chart_dir.name] = self.exposure_analyzer.analyze_chart(chart_dir, repo_path)
        ...
        for chart_name, chart_info in self.chart_containers.items():
            if chart_info["containers"]:
                ...
                filtered_containers = self.exposure_analyzer.filter_redundant_base_containers(chart_info["containers"], chart_name)
                self.results["containers"].extend(filtered_containers)
        
        # Reconstruct containers from dependencies
        self.results["containers"] = self.dependency_analyzer.reconstruct_containers_from_dependencies(
            self.chart_containers, self.results["containers"], repo_path
        )
        
        # Detect internal containers
        self.results["containers"] = self.dependency_analyzer.detect_internal_containers(
            self.chart_containers, self.results["containers"], repo_path
        )
        ...
        # Build master paths
        self.results["master_paths"] = self.path_consolidator.build_master_paths(self.results["containers"])
        
        # Build network topology
        self._build_network_topology()
        
        # Build Mermaid diagram
        self.results["mermaid_diagram"] = self.mermaid_builder.build_diagram(self.results["containers"])
    ...
    def _enrich_containers_with_dockerfile_info(self, repo_path: Path):
        ...
        for container in self.results["containers"]:
            ...
            # Find Dockerfile
            dockerfile_path = self.dockerfile_analyzer.find_dockerfile(chart_dir, repo_path, chart_name)
            if dockerfile_path:
                container["dockerfile_path"] = str(dockerfile_path.relative_to(repo_path))
                container["source_code_path"] = self.dockerfile_analyzer.parse_dockerfile_for_source_paths(dockerfile_path, repo_path)
    ...
    def _build_summary(self):
        ...
        summary = {
            ...
            "high_exposure": len([c for c in containers if c["exposure_level"] == "HIGH"]),
            "medium_exposure": len([c for c in containers if c["exposure_level"] == "MEDIUM"]),
            "low_exposure": len([c for c in containers if c["exposure_level"] == "LOW"]),
